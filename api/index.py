...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        target = channels.get(target_id) if target_id else proxy_url
        ...
        try:
            res = requests.get(target, headers=headers, stream=True, timeout=10)
            ...
            if ".m3u8" in target:
                ...
                for line in content.splitlines():
                    if line.startswith('#') or not line.strip():
                        ...
                        new_content += f"https://{host}/api/index?proxyUrl={seg_url}\n"
